    ...
    // Fetch Custom Links for Footer
    $stmt = $pdo->prepare("SELECT * FROM links WHERE location = 'footer' ORDER BY sort_order ASC");
    $stmt->execute();
    $footer_links = $stmt->fetchAll();
    ?>
    <footer class="<?php echo $footer_class; ?> mt-auto py-12 text-center">
        <div class="max-w-7xl mx-auto px-6">
            <div class="mb-8 flex justify-center space-x-8">
                <?php foreach($footer_links as $link): ?>
                    <a href="<?php echo htmlspecialchars($link['url']); ?>" class="text-sm font-bold uppercase tracking-widest hover:opacity-70"><?php echo htmlspecialchars($link['title']); ?></a>
                <?php endforeach; ?>
            </div>
            ...
            <div class="space-y-4">
                <p class="text-sm font-medium tracking-wide">
                    ...
                    <?php else: ?>
                        &copy; <?php echo date('Y'); ?> <b><?php echo htmlspecialchars($settings['footer_name'] ?? ($settings['site_name'] ?? 'Showcase')); ?></b>
                    <?php endif; ?>
                    by <a href="https://sglprime.com" target="_blank" class="underline hover:text-blue-400">sglprime.com</a>.
                </p>
                <p class="text-xs opacity-50 mt-2">© <?php echo date('Y'); ?> <?php echo htmlspecialchars($settings['footer_name'] ?? ($settings['site_name'] ?? 'Showcase')); ?>. Toate drepturile rezervate.</p>
            </div>
        </div>
    </footer>
</body>
